...
def search_state_vars_in_srclines(srclines):
    ...
    for line in srclines:
        ...
        # find scalar vars
        so_sc = re.search('^ *d([a-zA-Z0-9_]+)/dt[ \t]*=|^ *([a-zA-Z0-9_]+)\'[ \t]*=|^ *aux +([a-zA-Z0-9_]+) *=', line,
                          flags=re.IGNORECASE)

        # find array vars
        so_ar = re.search(
            '^ *([a-zA-Z0-9_]+)(\[[0-9]+\.\.[0-9]+\])\'[ \t]*=|^ *aux +([a-zA-Z0-9_]+)(\[[0-9]+\.\.[0-9]+\]) *=', line,
            flags=re.IGNORECASE)
...
def read_pars_values(srclines, pars_names=None):
    ...
    i_par_lines = np.nonzero(
        [re.search('^ *(parameters|par|param|params|p) (.+)$', line, flags=re.IGNORECASE) is not None for line in
         srclines])[0]
...
def change_parameters_in_ode_and_save(srclines, parameters, newfilepath):
    ...
    """
    lparameters = {pn.lower(): (lambda x: repr(x) if not isinstance(x, str) else x)(pv) for pn, pv in
...
def read_init_values(srclines, init_names=None):
    ...
    for i in i_init_lines:
        ...
        vars_list_sc2 += re.findall('([a-z0-9_]+ *\( *0 *\)) *= *([0-9\.e\-\+]+)', srclines[i].lower(),
                                    flags=re.IGNORECASE)
        vars_list_ar1 += re.findall('([a-z0-9_]+\[[0-9]+\.\.[0-9]+\] *\( *0 *\)) *= *([0-9\.e\-\+]+)',
                                    srclines[i].lower(), flags=re.IGNORECASE)
        vars_list_ar2 += re.findall('([a-z0-9_]+\[[0-9]+\.\.[0-9]+\]) *= *([0-9\.e\-\+]+)', srclines[i].lower(),
                                    flags=re.IGNORECASE)
...
def change_inits_in_ode_and_save(srclines, inits, newfilepath):
    ...
    i_d_line = np.nonzero([re.search('^ *(d)|^ *(done)', line, flags=re.IGNORECASE) is not None for line in srclines])[
        0]
    ...
    nsrclines = srclines[:]
    for i in i_par_lines:
        nsrclines[i] = re.sub('([a-z0-9_]+)( *\( *0 *\) *)?( *= *)([0-9\.e\-\+]+)', repl_in_par, nsrclines[i],
                              flags=re.IGNORECASE)

    for i in i_ar_lines:
        nsrclines[i] = re.sub('([a-z0-9_]+)\[([0-9]+..[0-9]+)\]( *\( *0 *\) *)?( *= *)([0-9\.e\-\+]+)', repl_in_ar,
                              nsrclines[i], flags=re.IGNORECASE)
...
def xpprun(filepath, version=8, xppname='xppaut', postfix='_tmp', parameters=None, inits=None, clean_after=False):
    ...
    if version < 8:
        ...
        try:
            res = subprocess.check_output("%s %s -silent -outfile %s" % (xppname, fullfilename, outputfilepath),
