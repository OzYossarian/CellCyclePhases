...
class TemporalNetwork:
    ...
    def from_static_network_and_edge_list_dataframe(
        ...
        # ToDo: applying this lambda is VERY slow - perhaps instead convert static_network.edges into a pandas
        # dataframe and join on it? Should be faster.
        missing_edges_indices = edges.apply(lambda edge: not static_network.has_edge(edge['i'], edge['j']), axis=1)
    ...
    def from_static_network_and_node_list_dataframe(
        ...
        missing_nodes_indices = nodes.apply(lambda node: not static_network.has_node(node['i']), axis=1)
